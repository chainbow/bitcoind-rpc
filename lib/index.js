    ...
    const req = this.protocol.request( options, function ( res ) {
      ...
      res.on( 'end', function () {
        ...
        if ( res.headers[ 'content-type' ] !== 'application/octet-stream' ) {
          const error = new Error( errorMessage + buf.toString( 'utf-8' ) );
        ...
        if ( res.statusCode === 401 ) {
          const error = new Error( errorMessage + 'Connection Rejected: 401 Unnauthorized' );
          if ( callback ) callback( error );
          reject( error );
          return;
        }
        if ( res.statusCode === 403 ) {
          const error = new Error( errorMessage + 'Connection Rejected: 403 Forbidden' )
          if ( callback ) callback( error );
          reject( error );
          return;
        }
        if ( res.statusCode === 500 && buf.toString( 'utf8' ) === 'Work queue depth exceeded' ) {
          const exceededError = new Error( 'Bitcoin JSON-RPC: ' + bufferWriter.toBuffer().toString( 'utf8' ) );
          exceededError.code = 429; // Too many requests
          if ( callback ) callback( exceededError );
